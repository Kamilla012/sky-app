...
const SatelliteMap = () => {
  ...
  // Ustaw domyślną wartość dla selectedSatelliteId
  useEffect(() => {
    if (simplifiedSatelliteList.length > 0) {
      setSelectedSatelliteId(simplifiedSatelliteList[0].satelliteId);
    }
  }, [simplifiedSatelliteList]);
  ...
  return (
    <div className='flex flex-col w-2/4'>
      {satelliteData ? (
        <MapContainer
          center={[satelliteData.geodetic.latitude, satelliteData.geodetic.longitude]}
          zoom={1}
        ...
        >
          ...
          />
          <Marker
            position={[satelliteData.geodetic.latitude, satelliteData.geodetic.longitude]}
            icon={L.icon({ iconUrl: satelliteImage, iconSize: [42, 42] })}
          >
            <Popup>
              <div>
      ...
      <label htmlFor="satelliteSelect" className='text-[white] text-[25px] text-center'>
        Select a satellite
      </label>
      <select
        id="satelliteSelect w-full"
        onChange={handleSatelliteChange}
        value={selectedSatelliteId}
        className={`${styles.inputForm}  text-center bg-gradient-to-r from-green-500 to-teal-500 text-white`}
      >
